...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  if (!chat.nsfw)
    throw `🔥 𝘽𝘼𝙆𝘾𝙃𝙊𝘿 𝘽𝙊𝙏 🔥  does not support NSFW.\n\nTo turn it on, use: ${usedPrefix}on nsfw \n\n🤤🤤🤤🤤🤤🤤🤤🤤🤤`;

  let user = global.db.data.users[m.sender].age;
  if (user < 17)
    throw `Lawde,18+ bkl. 😁 Age must be 18`;

  if (!text)
    throw `🔥 𝘽𝘼𝙆𝘾𝙃𝙊𝘿 𝘽𝙊𝙏 🔥 , What do you want to search on 🔥 𝘽𝘼𝙆𝘾𝙃𝙊𝘿 𝘽𝙊𝙏 🔥?\n🕸️ Usage: ${usedPrefix + command} <search>\n\nEx:Mia hue hue 🤣  or bza can use a link \nEx: .xnxx link *`;

  m.react(rwait);

  if (text.includes('http://') || text.includes('https://')) {
    if (!text.includes('xnxx.com'))
      return m.reply(`Invalid link. Only xnxx.com, Generated 🔥 𝘽𝘼𝙆𝘾𝙃𝙊𝘿 𝘽𝙊𝙏 🔥`);
...
🕸️ QUEEN-HENTAI
    ...
    try {
      ...
      if (searchJson.data && searchJson.data.length > 0) {
        ...
        m.reply(` No results found for the 🔥 𝘽𝘼𝙆𝘾𝙃𝙊𝘿 𝘽𝙊𝙏 🔥 search list.`);
      }
    } catch (e) {
